...
Dig = 9; % the distance between the HAP and the Gateways
Diu = 10; % the distance between the HAP and the Users
...
% the set of transmit power at the HAP
PHdBm = 5:5:35; % dBm
PHSet = 1e-3 .* 10.^(PHdBm./10); % the set of transmit powers of the HAP
...
for ii = 1: length(PHSet)
    ...
    countNumEB = 0;
    countNumET = 0;
    for i =1: Count % to control the number of random channel generation
        ...
        % compute the SNR as shown in the constraint C1 or C6
        gammaIG = zeros(N,1);
        for j =1:N
            % based on the result shown in Lemma 1.
        ...
        %%% golden search method to find the optimal EH time, i.e., b
        % the duration of EH phase is search between 0 to 1.
        mm = 0; % the minimum initial point
        nn = 1; % the maximum initial point
        cc = (-1+sqrt(5))/2; % step size

        % the loop of the golden search method.
        while 1
       
            
            % x1 and x2 are used to update the search range of the duration of EH phase,i.e., b
            x1 = nn - cc * (nn-mm);
            x2 = mm + cc * (nn-mm);
            ...
            % the function "BackThenForward" is the proposed algorithm (Algorithm 1) to solve P4, and obtain the optimal solution based on Eqs.(1)-(3).
                ...
                % R1 and R2 are the maximum achievable rates with the optimal solution
            ...
            % flag1 ==1 or flagu ==1, it indicates that the biection
            % method cannot find the optimal solution. So, we ignore the
            % results for this iteration (channel generation).
            if flagl ==1 || flagu ==1
                RProSumM5(i,ii) = 0;
                countNumPro = countNumPro +1; % to compute the total times that the bisection method fails
                break; % ignore this iteration.
            end
            
            % update the minimum point or the maximum point
            if R1 <= R2
                mm = x1;
            else
                nn = x2;
        ...
        % the CVX (Please refer to http://cvxr.com/cvx/ for more details) is used to compute the optimal time allocation directly.
        cvx_begin quiet
...
% the set to store all obtained results for each transmit tpower and each
% channel generations for the proposed scheme
RProSumM10 = zeros(Count,length(PHdBm));
% the set to store all obtained results for each transmit tpower and each
% channel generations for the benchmark with random energy beamforming
REBSumM10 = zeros(Count,length(PHdBm));
% the set to store all obtained results for each transmit tpower and each
% channel generations for the benchmark with equal time allocation
RETSumM10 = zeros(Count,length(PHdBm));

% the set to store the average results with M=5 for the proposed scheme
RProAveM10 = zeros(1, length(PHSet));
% the set to store the average results with M=5 for the benchmark with random energy beamforming
REBAveM10 = zeros(1, length(PHdBm));
% the set to store the average results with M=5 for the benchmark with equal time allocation
RETAveM10 = zeros(1,length(PHdBm));
for ii = 1: length(PHSet)
    ...
    for i =1: Count
        ...
        while 1
            ...
            % the function "BackThenForward" is the proposed algorithm (Algorithm 1) to solve P4.
...
plot(PHdBm, RProAveM5,'r-s','LineWidth',2);
hold on;
plot(PHdBm, REBAveM5, 'k:s','LineWidth',2);
plot(PHdBm, RETAveM5, 'b-.s','LineWidth',2);
plot(PHdBm, RProAveM10,'r-o','LineWidth',2);
plot(PHdBm, REBAveM10 , 'k:o','LineWidth',2);
plot(PHdBm, RETAveM10, 'b-.o','LineWidth',2);
